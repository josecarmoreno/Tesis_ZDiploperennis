...
#leer el K error
k.error<- read.delim("/home/genconser/Documentos/JoseCarlos/DiversidadZdiploperennis/R/resultados/admixture/admixtureZeadiploperennis/zdiploperennis_Kerror.txt", header = F, sep = ":")
rownames(k.error)<- c("k=1", "k=2", "k=3", "k=4", "k=5", "k=6", "k=7", "k=8", "k=9", "k=10", "k=11")


#Hacer el plot para el K error
e.plot<- ggplot(data=k.error, aes(x = 1:11 , y=V2)) +
  geom_point() +
  geom_line()  +
  theme_light() +
  xlab("k") +
...
#leer el archivo Q 
Qval<-read.table(paste0("/home/genconser/Documentos/JoseCarlos/DiversidadZdiploperennis/R/resultados/admixture/diploperennis_data_filtered.10.Q"))
...
Qval<-cbind(Estado=samples_meta$ESTADO, Qval)
...
#Ordenar niveles de la columna que ggplot2 usa en x para que estén en el orden deseado
Qval_long$INDIV<- factor(Qval_long$INDIV,
                         levels = Qval_long$INDIV[order(Qval$Estado)])
