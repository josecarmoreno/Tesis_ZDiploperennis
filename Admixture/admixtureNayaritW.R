...
#leer el K error
k.error<- read.delim("C:/Users/RYZEN 5/Documents/Para_microsoft/Resultados/Admixture/admixtureNa/admixtureNa2/zdiploperennisNa_Kerror.txt", header = F, sep = ":")
rownames(k.error)<- c("k=1", "k=2", "k=3", "k=4", "k=5", "k=6", "k=7")

#Hacer el plot para el K error
e.plot<- ggplot(data=k.error, aes(x=1:7, y=V2)) + geom_point() + geom_line()
e.plot + xlab("k") + ylab("Error") + ggtitle("Prueba de error cruzado Nayarit") + theme_light()

###Hacer el plot para K=6

##Obtener metadatos
samples_meta<-read.delim("C:/Users/RYZEN 5/Documents/Para_microsoft/metadata/meta_Nayarit.txt", 
                         header=TRUE, sep="\t")

samples_names<-read.delim("C:/Users/RYZEN 5/Documents/Para_microsoft/data/diploperennis_dataNa.fam", header=FALSE, sep=" ") %>%
  select(., V2) %>%
  rename(., INDIV=V2)

#leer el archivo Q 
Qval<-read.table(paste0("C:/Users/RYZEN 5/Documents/Para_microsoft/Resultados/Admixture/admixtureNa/admixtureNa2/diploperennis_dataNa.3.Q"))
