...
#leer el K error
k.error<- read.delim("/home/genconser/Documentos/JoseCarlos/DiversidadZdiploperennis/R/resultados/admixture/admixtureJa/zdiploperennisJa_Kerror.txt", header = F, sep = ":")
rownames(k.error)<- c("k=1", "k=2", "k=3", "k=4", "k=5", "k=6", "k=7", "k=8")

#Hacer el plot para el K error
e.plot<- ggplot(data=k.error, aes(x=1:8, y=V2)) + geom_point() + geom_line()
e.plot + xlab("k") + ylab("Error")

###Hacer el plot para K=2

##Obtener metadatos
samples_meta<-read.delim("/home/genconser/Documentos/JoseCarlos/DiversidadZdiploperennis/R/data/datosZdiploperennis/datosJC/Populations/meta_Jalisco.txt", 
                         header=TRUE, sep="\t")

samples_names<-read.delim("/home/genconser/Documentos/JoseCarlos/DiversidadZdiploperennis/R/data/datosZdiploperennis/datosJC/Populations/diploperennis_dataJa.fam", header=FALSE, sep=" ") %>%
  select(., V2) %>%
  rename(., INDIV=V2)

#leer el archivo Q 
Qval<-read.table(paste0("/home/genconser/Documentos/JoseCarlos/DiversidadZdiploperennis/R/resultados/admixture/admixtureJa/diploperennis_dataJa.5.Q"))
